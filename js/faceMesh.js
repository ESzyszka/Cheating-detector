...
function onResultsFaceMesh(results) {
  ...
  if (results.multiFaceLandmarks) {
    for (const landmarks of results.multiFaceLandmarks) {
      const categories = landmarks.category;
      ...
      // Get the positions of the right and left eyes
      const rightEye = landmarks[FACEMESH_RIGHT_EYE[0]];
      const leftEye = landmarks[FACEMESH_LEFT_EYE[0]];

      // Calculate the horizontal position of the right eye
      const rightEyeX = rightEye[0].x * out2.width;

      // Compare the current position with the previous one
      if (rightEyeX > prevRightEyeX) {
        console.log('Person is looking right');
      } else {
        console.log('Person is looking left');
      }
